...
const cadastrarEndereco = async (req, res) => {
    ...
    try {
        ...
        const endereco = await knex('enderecos_usuario').insert({ id_usuario, logradouro, numero, cidade, uf, cep, bairro, complemento });
...
const listarEnderecos = async (req, res) => {
    ...
    try {
        ...
        const enderecos = await knex('enderecos_usuario').where({ id_usuario });
...
const obterEndereco = async (req, res) => {
    ...
    try {
        ...
        const endereco = await knex('enderecos_usuario').where({ id_endereco_usuario });
...
const excluirEndereco = async (req, res) => {
    ...
    try {
        ...
        const endereco = await knex('enderecos_usuario').where({ id_endereco_usuario }).del();
...
const atualizarEndereco = async (req, res) => {
    ...
    try {
        ...
        const enderecoEncontrado = await knex('enderecos_usuario').where({ id_endereco_usuario }).first();

        if (!enderecoEncontrado) {
            return res.status(400).json("Endereço não existe");
        }

        const usuarioEncontrado = await knex('enderecos_usuario').where({ id_usuario });
        
        if(usuarioEncontrado.length === 0){
            return res.status(400).json("Usuário não existe"); 
        } else {
            const enderecoAtualizado = await knex('enderecos_usuario').where({ id_endereco_usuario }).update({ id_usuario, logradouro, numero, cidade, uf, cep, bairro, complemento });
